...
class IBMQuantumInteractionLayer(nn.Module):
    ...
    def __init__(self, n_qubits, n_layers, ibm_backend='ibmq_qasm_simulator', use_session=True):
        super().__init__()
        ...
        try:
            ...
            backend = self.service.backend(ibm_backend)
            print(f"✓ Connected to IBM backend: {backend.name}")
            print(f"  Status: {backend.status().status_msg}")
            print(f"  Qubits: {backend.num_qubits}")
            print(f"  Pending jobs: {backend.status().pending_jobs}")
        except Exception as e:
            print(f"⚠ Error connecting to IBM backend: {e}")
            print(f"  Please run: QiskitRuntimeService.save_account(token='YOUR_IBM_TOKEN')")
            raise

        # Create PennyLane device using IBM backend
        self.dev = qml.device(
            'qiskit.remote',
            wires=n_qubits,
            backend=ibm_backend,
