...
class JetTorchDataModule(L.LightningDataModule):
    def __init__(
        ...
        Args:
            ...
            num_workers : int, optional
                Number of data loader workers. If None, uses 80% of CPU cores.
        """

        super().__init__()
        self.batch_size = batch_size
        
        # Determine number of workers (use config value or default to 80% of CPU cores)
        if num_workers is None:
            import os
            cpu_worker_fraction = kwargs.get('cpu_worker_fraction', 0.8)
            num_workers = max(1, int((os.cpu_count() or 1) * cpu_worker_fraction))
    ...
    def train_dataloader(self):
        """Training data loader with optimized worker settings"""
    ...
    def val_dataloader(self):
        """Validation data loader with optimized worker settings"""
        return TorchDataLoader(
            self.valid_dataset, 
            batch_size=self.batch_size, 
            shuffle=False,
    ...
    def test_dataloader(self):
        """Testing data loader with optimized worker settings"""
        return TorchDataLoader(
            self.test_dataset, 
            batch_size=self.batch_size, 
            shuffle=False,
