...
class FatjetEvents:
    ...
    def generate_events(self, num_events: int) -> ak.Array:
        ...
        if total_num_events == 0:
            raise ValueError("No events available to sample from")

        # If requested number of events is greater than available, sample with
        # replacement to avoid ValueError from random.sample. Otherwise sample
        # without replacement for diversity.
        if num_events > total_num_events:
            print(f"Warning: requested {num_events} events but only {total_num_events} available. Sampling with replacement.")
            indices = random.choices(range(total_num_events), k=num_events)
        else:
            indices = random.sample(range(total_num_events), num_events)
